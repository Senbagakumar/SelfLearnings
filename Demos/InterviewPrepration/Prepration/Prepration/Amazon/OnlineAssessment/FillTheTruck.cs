...
namespace Prepration.Amazon.OnlineAssessment
{
    class FillTheTruck
    {
        ...
        //https://leetcode.com/discuss/interview-question/981154/Amazon-or-OA-New-Grad-or-Fill-The-Truck

        public static long getMaxUnit(int num, List<int> boxes, int unitSize, List<int> unitsPerBox, long truckSize)
        {
            long cnt = (long)0;

            List<int> unitsMaxHeap = new List<int>();
            ...
            int k = 0;
            while (truckSize > 0 && unitsMaxHeap.Count > 0)
            {
                int units = unitsMaxHeap[k]; // get max units count
                cnt += (long)units;
                truckSize--;
                k++;
            }
