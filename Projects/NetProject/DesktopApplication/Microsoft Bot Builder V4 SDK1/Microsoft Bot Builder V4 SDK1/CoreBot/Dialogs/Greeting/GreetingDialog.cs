    ...
    public class GreetingDialog : ComponentDialog
    {
            ...
            : base(nameof(GreetingDialog))
        {
            ...
            var waterfallSteps = new WaterfallStep[]
            {
                    ...
                    PromptForChoiceStepAsync,
                    PromptForConfirmChoiceAsync,
            };
            ...
            AddDialog(new TextPrompt("Confirmation"));
        ...
        private async Task<DialogTurnResult> PromptForChoiceStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            var choiceState = await UserProfileAccessor.GetAsync(stepContext.Context);

            //if(choiceState!=null)
            var listOfChoices = new List<string>() { "Yes", "No" };
            //{
                var opts = new PromptOptions
                {
                    Choices= ChoiceFactory.ToChoices(listOfChoices),
                    //Choices = new List<Choice>() { new Choice() { Value="Yes" }, new Choice() { Value="No" } },
                    Prompt = MessageFactory.Text("Are you working in Quadrant"),
                    RetryPrompt=MessageFactory.Text("Please choose an option from the list"),
                };
                return await stepContext.PromptAsync(ChoicePrompt, opts, cancellationToken);
            //}
            //return await stepContext.NextAsync();
        }
