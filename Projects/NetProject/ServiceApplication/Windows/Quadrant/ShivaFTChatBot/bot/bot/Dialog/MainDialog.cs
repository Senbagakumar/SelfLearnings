...
namespace bot.Dialogs
{
    public class MainDialog : ComponentDialog
    {
        ...
        public MainDialog(ILogger<MainDialog> logger, CertificationDialog applicationdialog, PermissionDialog helperdialog, MeetingDialog meetingdialog)
        ...
        private async Task<DialogTurnResult> IntroAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                return await stepContext.NextAsync(stepContext.Options);
            }
        }
        ...
        private async Task<DialogTurnResult> PromptsAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (x == "1" || x.Contains("Permission", StringComparison.OrdinalIgnoreCase))
            {
                return await stepContext.BeginDialogAsync(nameof(PermissionDialog), cancellationToken);
            }
            else if (x == "2" || x.Contains("Certifications", StringComparison.OrdinalIgnoreCase))
            {
                return await stepContext.BeginDialogAsync(nameof(CertificationDialog), cancellationToken);
            }
